...
EncoderProgressBlock encodeProgressBlock;
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    double bitrate;
    double speed;
    int64_t pts = INT64_MIN + 1, duration_5 = INT64_MIN + 1;
    ...
    secsTotal = FFABS(pts) / AV_TIME_BASE;
    secs = (int)secsTotal;
    ...
    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
             ...
             (100 * us) / AV_TIME_BASE);
    if (encodeProgressBlock) {
        encodeProgressBlock((long)total_size, (long)pts);
    }
...
int video_merge(int argc, char **argv, EncoderProgressBlock progressBlock, EncoderCompletionBlock completionBlock)
{
    ...
    //         exit_program(1);
    //     }
    encodeProgressBlock = progressBlock;
    ...
    ffmpeg_cleanup(0);
    encodeProgressBlock = NULL;
    completionBlock(0, "ok");
    return main_return_code;
}
int video_progress(int videoTotalTime)
{
    LOGI("video_progress secsTotal is %f, videoTotalTime is %d \n", secsTotal, videoTotalTime);
    int ret = round(secsTotal * 1000 * 100 / videoTotalTime);
    return ret > 100 ? 100 : ret;
}

int video_finish()
{
    LOGI("video_finish set_finish");
    set_finish = 1;
    return 0;
}
