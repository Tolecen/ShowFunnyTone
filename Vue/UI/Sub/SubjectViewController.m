...
#define BACKGROUND_COLOR_TO            ((MINIMAL_UI) ? BACKGROUND_COLOR_MINIMAL : BACKGROUND_COLOR_CLASSIC)
...
@interface SubjectViewController () <UIActionSheetDelegate,
...
- (instancetype)initWithSubjectId:(long)subjectId
{
    _subjectId = subjectId;
    NSString *subject = [NSString stringWithFormat:@"%@/subject/%ld?agent=ios&userId=%ld", HOST, subjectId, [Utils userId]];
    [self initSubjectInfo];
    return [self initWithURL:[NSURL URLWithString:subject]];
}

- (instancetype)initWithSubject:(SubjectInfo *)mSubjectInfo
{
    _mSubjectInfo = mSubjectInfo;
    _subjectId = _mSubjectInfo.activity_id;
    NSString *subject = [NSString stringWithFormat:@"%@/subject/%ld?agent=ios&userId=%ld", HOST, _subjectId, [Utils userId]];
    return [self initWithURL:[NSURL URLWithString:subject]];
}
...
- (void)initSubjectInfo
{
    ...
    NSString *fields = [NSString stringWithFormat:@"{\"subject_id\":\"%ld\",\"uid\":\"%ld\"}", _subjectId, [self loginUserId]];
    [souce getData:SUBJECT_INFO parameters:fields completion:completionBlock];

}

- (void)processDetailsData:(NSDictionary *)data
{
    SubjectInfoInfo *mInfo = [SubjectInfoInfo mj_objectWithKeyValues:data];
    ...
    if (mInfo.msg > 0) {
        ...
        self.mSubjectInfo = mInfo.info;
        self.title = self.mSubjectInfo.activity_title;
    }
}
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    view.backgroundColor = (self.hideWebViewBoundaries ? [UIColor whiteColor] : BACKGROUND_COLOR_TO);
...
- (void)setUpNavigationButtons
{
    ...
    if (self.backButton == nil) {
        ...
        self.backButton.tintColor = self.buttonBottomTintColor;
    }
    ...
    if (self.forwardButton == nil) {
        ...
        self.forwardButton.tintColor = self.buttonBottomTintColor;
    }
    ...
    if (self.reloadStopButton == nil) {
        ...
        self.reloadStopButton.tintColor = self.buttonBottomTintColor;
    }
    ...
    if (self.showActionButton && self.actionButton == nil) {
        ...
        self.actionButton.tintColor = self.buttonBottomTintColor;
...
//    [MobClick beginLogPageView:@"活动营销页面"];
}
...
//    [MobClick endLogPageView:@"活动营销页面"];
}
...
{
    if (buttonBottomTintColor == _buttonBottomTintColor)
        return;
    
    _buttonBottomTintColor = buttonBottomTintColor;
    
    if (MINIMAL_UI) {
        self.backButton.tintColor = _buttonBottomTintColor;
        self.forwardButton.tintColor = _buttonBottomTintColor;
        self.reloadStopButton.tintColor = _buttonBottomTintColor;
        self.actionButton.tintColor = _buttonBottomTintColor;
    }
    else {
        if (self.buttonThemeAttributes == nil)
            self.buttonThemeAttributes = [NSMutableDictionary dictionary];
        
        self.buttonThemeAttributes[TOWebViewControllerButtonTintColor] = _buttonBottomTintColor;
        [self setUpNavigationButtons];
    }
}
...
- (void)showPlaceholderTitle
{
    ...
    if (self.url && self.showPageTitles && self.showUrlWhileLoading) {
        ...
        if (_mSubjectInfo != nil) {
            self.title = _mSubjectInfo.activity_title;
        } else {
            self.title = @"萌宝拍";
        }
...
//    [MobClick event:@"shareSubject"];
    NSString *des = self.mSubjectInfo.activity_des;
    ...
    __block SubjectViewController *wSelf = self;
    if (_mSubjectInfo.file_id > 0) {
        SDWebImageManager *manager = [SDWebImageManager sharedManager];
        [manager downloadImageWithURL:[Utils getImagePathURL:_mSubjectInfo.file tagWith:IMAGE_NOR] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            
        } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                shareImage = image;
                [wSelf shareCommon:wSelf.title shareDes:des shareUrl:[NSString stringWithFormat:@"%@/subject/%ld", HOST, _subjectId] shareImageUrl:imageUrl shareImage:shareImage];
            });
        }];
    } else {
        [self shareCommon:self.title shareDes:des shareUrl:[NSString stringWithFormat:@"%@/subject/%ld", HOST, _subjectId] shareImageUrl:imageUrl shareImage:shareImage];
    }
}
